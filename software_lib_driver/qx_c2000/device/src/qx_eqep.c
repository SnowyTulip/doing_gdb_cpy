...
void EQEP_setCompareConfig(uint32_t base, uint16_t config, uint32_t compareValue, uint16_t cycles)
{
    ...
    HWREGH(base + EQEP_O_QPOSCTL)
        = (HWREGH(base + EQEP_O_QPOSCTL) & ~(EQEP_QPOSCTL_PCSPW_M | EQEP_QPOSCTL_PCLOAD | EQEP_QPOSCTL_PCSHDW))
          | regValue;
    ...
    HWREGH(base + EQEP_O_QDECCTL)
        = (HWREGH(base + EQEP_O_QDECCTL) & ~(EQEP_QDECCTL_SOEN | EQEP_QDECCTL_SPSEL)) | regValue;
}
