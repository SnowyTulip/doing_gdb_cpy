...
void usb_dbg_test() {
 ...
 //added by wangwk 17-03-14
  uint32_t npc_address=0x007f0c0c;
  uint32_t ppc_address=0x007f0c10;
  ...
  printf("Read      npc address = %.8x ppc address = %.8x r2 address = %.8x\n", npc_address, ppc_address, r2_address);
  printf("Read      npc = %.8x ppc = %.8x r2 = %.8x\n", npc, ppc, r2);
...
int usb_dbg_command(uint32_t type, uint32_t adr, uint32_t len) {
 ...
 try_again:
  ...
  if (DEBUG_GDB_DUMP_DATA && ( reg_adr < 0x001f0e00 || reg_adr > 0x001f0fec)) 
      printf("\nsend out comm type:%d len:%d addr:0x%x(type 6-read 2-write) \n", type,len,adr);
...
int usb_dbg_go(unsigned char *data, uint16_t len, uint32_t read) {
 ...
 try_again:
  //printf("dbg_chain:%d\n",dbg_chain);
    ...
    printf("go len is %dbytes, read=%d (if 0, writing)\n\n", len, read);
