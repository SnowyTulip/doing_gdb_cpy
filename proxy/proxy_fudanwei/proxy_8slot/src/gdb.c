...
#define MAX_GPRS    (1068)  
#define MAX_GPRS_all    (1328)  
#define GR_START	(0x007f0e00)
#define GR_NUM		(32)
#define	VR_START	(0x007f1000)
#define VR_NUM		(16)
#define VR_LENGTH	(2560)	//VR有160位宽度
#define VR_GAP		(0x200)	//相邻VR之间的地址差异
#define MOB_START	(0x007f3010)
...
#define NPC_CPU_REG_ADD  	0x007f0c0c	/* Next PC */
#define SR_CPU_REG_ADD   	11	/* Supervision Register */
#define PPC_CPU_REG_ADD  	0x007f0c10	/* Previous PC */
#define EXPC_CPU_REG_ADD	0x007f0c14	/* Previous PC */
#define DMR1_CPU_REG_ADD	(0x007f0800 + (16<<2))	//0x001f0840	/* Debug Mode Register 1 (DMR1) 0x3010 */
#define DMR2_CPU_REG_ADD	(0x007f0800 + (17<<2))	//0x001f0844	/* Debug Mode Register 2 (DMR2) 0x3011 */
#define DSR_CPU_REG_ADD		(0x007f0800 + (20<<2))	//0x001f0850	/* Debug Stop Register (DSR) 0x3014 */
#define DRR_CPU_REG_ADD  	(0x007f0800 + (21<<2))	//0x001f0854	/* Debug Reason Register (DRR) 0x3015 */
#define GR_CPU_REG_ADD  	(ADDR_SPR_BOOT+0x0e00)	/* GR */
#define VR_CPU_REG_ADD(N)	(0x007f1000 + (N<<9))

#define VR0_CPU_REG_ADD  	(ADDR_SPR_BOOT+0x1000)	/* VR0 */
#define VR1_CPU_REG_ADD      (VR0_CPU_REG_ADD + (1<<9))    /* VR1 */
#define VR2_CPU_REG_ADD      (VR0_CPU_REG_ADD + (2<<9))    /* VR2 */
#define VR3_CPU_REG_ADD      (VR0_CPU_REG_ADD + (3<<9))    /* VR3 */
#define VR4_CPU_REG_ADD      (VR0_CPU_REG_ADD + (4<<9))    /* VR4 */
#define VR5_CPU_REG_ADD      (VR0_CPU_REG_ADD + (5<<9))    /* VR5 */
#define VR6_CPU_REG_ADD      (VR0_CPU_REG_ADD + (6<<9))    /* VR6 */
#define VR7_CPU_REG_ADD      (VR0_CPU_REG_ADD + (7<<9))    /* VR7 */
#define VR8_CPU_REG_ADD      (VR0_CPU_REG_ADD + (8<<9))    /* VR8 */
#define VR9_CPU_REG_ADD      (VR0_CPU_REG_ADD + (9<<9))    /* VR9 */
#define VR10_CPU_REG_ADD      (VR0_CPU_REG_ADD + (10<<9))    /* VR10 */
#define VR11_CPU_REG_ADD      (VR0_CPU_REG_ADD + (11<<9))    /* VR11 */
#define VR12_CPU_REG_ADD      (VR0_CPU_REG_ADD + (12<<9))    /* VR12 */
#define VR13_CPU_REG_ADD      (VR0_CPU_REG_ADD + (13<<9))    /* VR13 */
#define VR14_CPU_REG_ADD      (VR0_CPU_REG_ADD + (14<<9))    /* VR14 */
#define VR15_CPU_REG_ADD      (VR0_CPU_REG_ADD + (15<<9))    /* VR15 */

#define DVR_CPU_REG_ADD(N)  	(0x007f0800 + (N<<2))	

//#define DVR1_CPU_REG_ADD  	(0x001f0800 + (1<<2))	
//#define DVR2_CPU_REG_ADD  	(0x001f0800 + (2<<2))	
//#define DVR3_CPU_REG_ADD  	(0x001f0800 + (3<<2))	
//#define DVR4_CPU_REG_ADD  	(0x001f0800 + (4<<2))	
//#define DVR5_CPU_REG_ADD  	(0x001f0800 + (5<<2))	
//#define DVR6_CPU_REG_ADD  	(0x001f0800 + (6<<2))	
//#define DVR7_CPU_REG_ADD  	(0x001f0800 + (7<<2))	

#define DCR_CPU_REG_ADD(N)  	(0x007f0800 + ((N +8)<<2))	

//#define DCR1_CPU_REG_ADD  	(0x001f0800 + (9<<2))	
//#define DCR2_CPU_REG_ADD  	(0x001f0800 + (10<<2))	
//#define DCR3_CPU_REG_ADD  	(0x001f0800 + (11<<2))	
//#define DCR4_CPU_REG_ADD  	(0x001f0800 + (12<<2))	
//#define DCR5_CPU_REG_ADD  	(0x001f0800 + (13<<2))	
//#define DCR6_CPU_REG_ADD  	(0x001f0800 + (14<<2))	
//#define DCR7_CPU_REG_ADD  	(0x001f0800 + (15<<2))	
...
int gdb_fd = 0;
u_int32_t oldaddr;
char buffer_data[GDB_BUF_MAX_TIMES_TWO];
...
static void rsp_step_continue(struct rsp_buf *p_buf);
static void rsp_hardware_continue(struct rsp_buf *p_buf);
...
static void rsp_read_mem_seprate(struct rsp_buf *p_buf);
...
static int send_file_request(int trapImm);
static int target_recv_trap_back(struct rsp_buf *p_buf);
...
void rsp_init(void)
{
	...
	stallState = STALLED;
	printf("stallState == STALLED\n");
	uint32_t temp_uint32;
	gdb_set_chain(SC_WISHBONE);
	unsigned int addr = 0x200400;
	char word_tem[5]={'\0'};
...
int current_stat=0;//现在在跑肯定是因为gdb发了s或c，指示现在的状态是s或c或stop
	//0--origin 1--s 2--c
int current_filefunc = 0;//现在是哪个fileio函数在作用，用于接收到回复信息的函数
int traping=0;
...
//	uint32_t cnt = 0;	
	while (1) {
		...
		if (rsp.client_waiting) {
			...
			{	// Get the EXPC if we're stalled
				gdb_set_chain(SC_RISC_DEBUG);
				gdb_read_reg(NPC_CPU_REG_ADD, &temp_uint32);//zsy change npc to expc
				if(SYS_OUT)
				printf("NPC = %x\n", temp_uint32);
			}
			if( TARGET_SIGNAL_USR2 == rsp.sigval){
				if(DEBUG_GDB)
					printf("trap occurs, the trap imm is %d\n",rsp.trap_imm);
				rsp.client_waiting = 0;
			}else if ((TARGET_SIGNAL_TRAP == rsp.sigval)
			    && (NULL !=
...
//				set_npc(temp_uint32);
			...
			else if((TARGET_SIGNAL_TRAP == rsp.sigval) && (stallState != STALLED))
			{
				rsp_report_exception();
				rsp.client_waiting = 0;	/* No longer waiting */
			}
			else if (cnt == targetcnt) {//循环100次认为程序结束了
				printf("cnt=%d\n",cnt);
				rsp_report_exception();
				rsp.client_waiting = 0;	/* No longer waiting */
				cnt=0;
        	...
        	if(tmptime.tv_sec>time1.tv_sec||(tmptime.tv_usec/1000)>(time1.tv_usec/1000)){
            	break;
        	}
    		}
			}
...
static void rsp_client_request(void)
{
	...
	int i = 0;
	if(DEBUG_GDB)
		printf("rsp_client_request received ");
	if(DEBUG_GDB)
	{
	...
	if (DEBUG_GDB) {
		...
		if(p_buf->data[0]=='G')
		{
			printf("Packet received G: %d chars\n", 
		       p_buf->len);
			fflush(stdout);
		}
		else
...
//	printf("rsp_continue_generic () set_stall_state ... \n");
	...
	printf("Ater unstall the processor stalled = %d\n",stalled);
/* Note the GDB client is now waiting for a reply. */
	rsp.client_waiting = 1;   //annotated by wangwk 17-05-22
//	rsp.client_waiting = 0;  // added by wangwk 17-05-22
	cnt = 0;
	printf("waiting = %d\n", rsp.client_waiting);
	...
	for (r = 0; r < MAX_GPRS_all; r++) {
		if(r < GR_NUM)//读GR寄存器
		{
			...
			err = gdb_read_reg(address, &temp_uint32);
			 if(r == 31)
			 	temp_uint32 = temp_uint32 + 0x200000;// + 0x400;// - 0x400;
			
			//  if(r == 30)
			//  	temp_uint32 = temp_uint32 + 0x200000 - 0x400;
			if (err > 0) {
				if (DEBUG_GDB)
		...
		else//读MOB类寄存器
		{
			...
			if(DEBUG_GDB_DUMP_DATA)
				printf("address=0x%x,r=%d\n",address,r);
			err = gdb_read_reg(address, &temp_uint32);
			if(temp_uint32<0x00200000){
				temp_uint32 += 0x00200000 ;
			}
...
//	len = 4;//171221
	...
	if (addr & 0x3)		// address not word-aligned, do byte accesses first
	{
		...
		addr_tem=addr;
		addr=addr-(addr&0x3);
		if (addr >= 0x200000 && intr==0) {
			//addr += 0x400;
			if (addr % 32 == 0)
				addr = (addr / 32) * 32 + 0x1c;
			else if (addr % 32 == 4)
				addr = (addr / 32) * 32 + 0x18;
			else if (addr % 32 == 8)
				addr = (addr / 32) * 32 + 0x14;
			else if (addr % 32 == 0xc)
				addr = (addr / 32) * 32 + 0x10;
			else if (addr % 32 == 0x10)
				addr = (addr / 32) * 32 + 0xc;
			else if (addr % 32 == 0x14)
				addr = (addr / 32) * 32 + 0x8;
			else if (addr % 32 == 0x18)
				addr = (addr / 32) * 32 + 0x4;
			else if (addr % 32 == 0x1c)
				addr = (addr / 32) * 32 + 0x0;
		...
		err = gdb_read_block(addr, (uint32_t *) word_tem,bytes_per_word);
		if (err) 
		{
			put_str_packet("E01");
			return;
		...
		char tem;
		tem = word_tem[0];
		word_tem[0] = word_tem[3];
		word_tem[3] = tem;
		tem = word_tem[1];
		word_tem[1] = word_tem[2];
		word_tem[2] = tem;
		//取出byte赋给rec_buf
	...
	if (len_cpy / bytes_per_word)	// Now perform all full word accesses
	{
		...
		{	
			...
			//printf("addr=0x%x\n",addr);
			err = gdb_read_block(addr, (uint32_t *) rec_buf_ptr,bytes_per_word);
			//进行大小端转换
			char tem;
			tem = rec_buf_ptr[0];
			rec_buf_ptr[0] = rec_buf_ptr[3];
			rec_buf_ptr[3] = tem;
			tem = rec_buf_ptr[1];
			rec_buf_ptr[1] = rec_buf_ptr[2];
			rec_buf_ptr[2] = tem;
			if (err) {
				put_str_packet("E01");
				return;
...
static void rsp_step_generic(uint32_t addr, uint32_t except)
{
	...
	//set_npc(addr);	//annotated by wangwk 17-05-22
	/* Clear Debug Reason Register (DRR) 0x3015 */
	// Arc sim --> cpu_state.sprs[SPR_DRR]   = 0;
	if (gdb_write_reg(DRR_CPU_REG_ADD, 0))
		printf("Error write to DRR register\n");
	...
	/* Unstall the processor */
	set_stall_state(0);   //annotated by wangwk 17-05-22
	unsigned char stalled;   //annotated by wangwk 17-05-22
	dbg_cpu0_read_ctrl(0, &stalled);  // annotated by wangwk 17-05-22
	if (DEBUG_GDB)
	printf("Ater unstall the processor stalled = %d\n",stalled);
...
//	addr += 0x00100000 - 0x100;
	...
	if (len) {
		if (addr >= 0x00200000) {
			uint32_t padsize=addr%32;
			if (padsize){
				char* tmp=malloc(8*bytes_per_word); //read a block
				gdb_read_block(addr-padsize,tmp,8*bytes_per_word); // dont know why?
				for (int i=0;i<padsize;++i) buffer_data[i]=tmp[8*bytes_per_word-i-1];
				printf("read padding memory: \n");
				for (int i=0;i<8;++i){
				...
				free(tmp);
				addr-=padsize;
				memcpy(buffer_data+padsize,bindat,len);
				bindat=buffer_data;
				len+=padsize;
			}
			swap_buf_256bit(bindat, &len);
			oldaddr=addr+len;
			//addr = 0x00200400;
		}
...
//	switch (type) {
	...
	case BP_HARDWARE:	// hardware-breakpoint Z1  hbreak
		...
		if (NULL == mp_hash_lookup(type, addr)) {
			gdb_ensure_or1k_stalled();
...
// static int send_file_request(int trapImm)
// {
// 	 char *Imm2func[] = {"NULL","open1", "open2", "close", "read", "write",
...
//     //continue to run
//     printf("current_stat=%s\n",current_stat==1?"step":"continue");
...
static int send_file_request(int trapImm)
{
    ...
    {	
        ...
        current_filefunc = open1_func_code;
        uint32_t path; 
		gdb_read_reg(GR_START + 4*4, &path);
		uint32_t flags;
		gdb_read_reg(GR_START + 5*4, &flags);
		uint32_t len=1;
		uint32_t tmppath=path;
		uint8_t tmp8;
		gdb_set_chain(SC_WISHBONE);
		printf("open path : \n");
		while(1){
			char word_tem[5]={'\0'};
			unsigned int addr_tem;
			addr_tem=tmppath-(tmppath&0x3);
			err = gdb_read_block(addr_tem, (uint32_t *) word_tem,bytes_per_word);
			printf("tmppath : %x,word_tem : %s\n",tmppath,word_tem);
			printf("%c\n",word_tem[0x3-(tmppath&0x3)]);
			if(word_tem[0x3-(tmppath&0x3)]==0){
				break;
			}
			else{
				len++;
    ...
    case open2_func_code: //int open2(const char *pathname, int length, int flags, unsigned int mode)
    {
		...
		gdb_read_reg(GR_START + 5*4, &flags);
        mode_t f_attrib;
		gdb_read_reg(GR_START + 6*4, &f_attrib);
		uint32_t len=1;
		uint32_t tmppath=path;
		uint8_t tmp8;
		gdb_set_chain(SC_WISHBONE);
		// gdb_read_byte(tmppath, (uint8_t *)&tmp8);
		// while(tmp8!=0x0){
		// 	len++;
		// 	tmppath++;
		// 	gdb_read_byte(tmppath, (uint8_t *)&tmp8);
    ...
    case read_func_code: //int read(int fd, void *buf, unsigned int count);
    {
		...
		gdb_read_reg(GR_START + 4*4, &fd);
        uint32_t buf;
		gdb_read_reg(GR_START + 5*4, &buf);
        unsigned int count;
		gdb_read_reg(GR_START + 6*4, &count);
        sprintf(packet, "Fread,%x,%x,%x", fd, buf, count);
    }
    break;
    case write_func_code: //int write(int fd, const void *buf, unsigned int count);
...
static int target_recv_trap_back(struct rsp_buf *p_buf)
{
    ...
    if (retcode[0]=='-') //fail find errorno
    {
        sscanf(p_buf->data, "F%*[^,],%x[^,]", &errorno);
        printf("system call error:%d\n", errorno);
		gdb_write_reg(GR_START + 2*4, errorno);
    } //ignore error, just run
    //write to GR2
    //write to GR30+0
	//gdb_set_chain(SC_RISC_DEBUG);
	...
	gdb_write_reg(GR_START + 2*4, ret);
    //cpu->mempart->store_word(addr,ret);
    //this->debug_store_region(addr, len, retcode, this);
    //continue to run
    printf("current_stat=%s\n",current_stat==1?"step":"continue");
