...
int main(int argc, char *argv[])
{
	...
	if (argc > 1)
	{
		...
		{
			targetcnt = strtol(argv[i+1], &s, 10);
		}
		else
		{
			print_usage();
...
int dbg_wb_write_block32(uint32_t adr, uint32_t * data, uint32_t len)
{
	...
	if (endpoint_target == ENDPOINT_TARGET_USB){
		...
		int res=0;
		while(tmplen>=1000){
			int tmpres=usb_dbg_wb_write_block32(tmpadr, tmpdata, 1000);
			res=res+tmpres;
			tmpdata=tmpdata+250;
			tmplen=tmplen-1000;
			tmpadr=tmpadr+1000;
		...
		if(tmplen>0){
			int tmpres=usb_dbg_wb_write_block32(tmpadr, tmpdata, tmplen);
			res=res+tmpres;
		}
		return res;
	}
...
#ifdef USB_ENDPOINT_ENABLED
	printf
	    ("\t\tStart a GDB server on port 50001, target cnt = 4000 using RSP, connecting to\n\t\thardware target via USB:\n\n");
	printf("\t\t./or_debug_proxy -r 50001 -c 4000\n");
	printf("\n");
#endif
	fflush(stdout);
}
