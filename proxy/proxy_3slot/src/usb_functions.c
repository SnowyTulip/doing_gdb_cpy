...
void usb_dbg_test() {
    ...
    //added by wangwk 17-03-14
    // 2023/08/02: npc and ppc copied from gdb.c
    uint32_t npc_address = 0x007f040c; //0x007f0c0c;
    uint32_t ppc_address = 0x007f0410; //0x007f0c10;
    uint32_t r2_address = 0x007f0e08;

    printf("usb_dbg_test(): stalling OR1K CPU0\n");
    err = dbg_cpu0_write_ctrl(0, 0x01);
    printf("usb_dbg_test(): dbg_cpu0_write_ctrl() returned with value: %d\n", err);
    err = dbg_cpu0_read_ctrl(0, &stalled);
    printf("usb_dbg_test(): dbg_cpu0_read_ctrl() returned with value: %d\n", err);
    ...
    if (!err) {
        printf("usb_dbg_test(): start reading NPC\n");
    ...
    if (!err) {
        err = dbg_cpu0_read(r2_address, &r2, 4);  /* Read R1 */
    }
    if (err) {
    ...
    printf("usb_dbg_test(): npc_address = 0x%.8x, ppc_address = 0x%.8x, r2_address = 0x%.8x\n", npc_address, ppc_address, r2_address);
    printf("usb_dbg_test(): npc = 0x%.8x, ppc = 0x%.8x, r2 = 0x%.8x\n", npc, ppc, r2);
...
int usb_dbg_cpu0_read(uint32_t adr, uint32_t* data, uint32_t length) {
    reg_adr = adr;
    if ((err = usb_dbg_set_chain(cpu0_or_cpu1))) return err;
